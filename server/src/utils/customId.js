 ...
 * @param {Array} format - Array of part descriptors stored in Inventory.customIdFormat
 * @param {number} sequenceValue - The next sequence number for this inventory
 * @returns {string}
 */
  ...
  return format
    .map((part) => {
      switch (part.type) {
        ...
        case 'datetime': {
          // Compact ISO date-time: 20240315143022
          const now = new Date();
...
/**
 * Preview a custom ID format with example values (no randomness).
 */
